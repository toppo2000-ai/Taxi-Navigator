...
const HistoryView: React.FC<HistoryViewProps> = ({ 
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        const [pubSnap, userSnap] = await Promise.all([
          getDoc(doc(db, "public_status", viewingUid)),
          getDoc(doc(db, "users", viewingUid))
        ]);
        
        // historyとmonthsデータはpublic_statusから取得
        if (pubSnap.exists()) {
          const pubData = pubSnap.data();
          setOtherHistory(pubData.history || []);
  ...
  const targetDayMetadata = isViewingMe ? myDayMetadata : otherDayMetadata;
  // ★追加: monthsDataも選択
  const targetMonthsData = isViewingMe ? myMonthsData : otherMonthsData;
        ...
        return (
          <div className="w-full bg-[#0A0E14] min-h-screen">
            <DailyDetailView 
              ...
              isMe={isViewingMe}
              onUpdateMetadata={isViewingMe ? onUpdateMetadata : undefined}
              onEditRecord={isViewingMe ? onEditRecord : undefined}
            />
          </div>
        );
      } catch (error) {
