...
export const MonthlyDashboard: React.FC<MonthlyDashboardProps> = ({
  ...
  const yearlyData = useMemo(() => {
    ...
    for (let m = 0; m < 12; m++) {
      ...
      if (cachedMonth && cachedMonth.records) {
        // ★キャッシュされたデータを使用（高速）
        monthRecords = cachedMonth.records || [];
        sales = cachedMonth.sales || 0;
  ...
  const tableMonthlyData = useMemo(() => {
    ...
    // ★最適化: monthsDataから該当月のrecordsを取得
    const safeMonthsData = monthsData || {};
    const cachedMonth = safeMonthsData[sortKey];
    const cachedRecords = cachedMonth?.records || [];
    
    // 当日分など最新のデータはhistoryから取得（monthsに反映されていない可能性があるため）
    const safeHistory = history || [];
    const today = new Date();
    const todayStr = getBusinessDate(today.getTime(), businessStartHour);
    
    // 当日のデータのみhistoryから取得してマージ
    const todayRecords = safeHistory.filter(r => {
      const bDate = getBusinessDate(r.timestamp, businessStartHour);
      return bDate === todayStr;
    });
    
    // キャッシュされたrecordsと当日分をマージ（重複排除）
    const allRecordsMap = new Map<string, SalesRecord>();
    cachedRecords.forEach((r: SalesRecord) => {
      if (getBusinessDate(r.timestamp, businessStartHour) !== todayStr) {
    ...
    const mergedRecords = Array.from(allRecordsMap.values());
    
    // 簡易モード優先でフィルタリング
    const allRecords = filterRecordsWithSimpleModePriority(mergedRecords, businessStartHour);
      ...
      <div className="space-y-4">
        <div className="flex bg-gray-800 p-1.5 rounded-2xl border-2 border-blue-500 shadow-sm">
          ...
          <button 
            ...
            className={`flex-1 py-3 rounded-xl font-black transition-all flex items-center justify-center gap-2 text-sm whitespace-nowrap ${viewMode === 'yearly' ? 'bg-[#1A222C] text-blue-400 shadow-md border border-gray-700' : 'text-gray-500'}`}
          >
                ...
                <table className="w-full text-center text-sm">
                  ...
                  <tbody className="text-gray-300 font-medium">
                    ...
                    <tr className="bg-[#EAB308] text-black font-black border-t-2 border-amber-600 text-base">
                      ...
                      <td className="py-3 px-1 text-sm" colSpan={2}>{tableMonthlyData.totals.workDays}出番</td>
                ...
                <table className="w-full text-center text-sm">
                  <thead>
                    <tr className="bg-[#004d00] text-white text-xs border-b border-green-800/50">
                      <th className="py-3 px-2 font-bold w-[20%]">月度</th>
                      ...
                      <th className="py-3 px-4 font-bold w-[35%] text-right">営収</th>
                    </tr>
                  </thead>
                  <tbody className="text-gray-200 font-medium">
                    {yearlyData.months.map((m, i) => (
                      ...
                      >
                        <td className="py-3 px-2 font-bold text-blue-400 border-r border-gray-800/50 underline decoration-blue-500/30">
                          {m.fullLabel}
                        </td>
                        ...
                        <td className={`py-3 px-4 text-right font-mono font-black tracking-tight ${m.sales > 0 ? 'text-white' : 'text-gray-700'}`}>
                    ...
                    <tr className="bg-[#FFE4B5] text-black font-black border-t-2 border-orange-400 text-base">
                      ...
                      <td className="py-3 px-4 text-right">{yearlyData.totals.sales.toLocaleString()}</td>
                    </tr>
                  </tbody>
                </table>
              )}
