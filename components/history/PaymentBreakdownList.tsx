...
import { Coins, CreditCard, Banknote, Smartphone, Ticket, QrCode } from 'lucide-react';
import { PaymentMethod, DEFAULT_PAYMENT_ORDER, SalesRecord } from '../../types';
import { PAYMENT_LABELS, getPaymentColorClass, formatCurrency } from '../../utils';
    ...
    <div className="space-y-4">
      {nonCashAmountTotal > 0 && (
        <div className="bg-gradient-to-r from-indigo-900/60 to-blue-900/60 p-4 rounded-2xl border border-indigo-500/30 flex justify-between items-center shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold text-indigo-200 block uppercase tracking-widest">
                キャッシュレス計
              </span>
      ...
      <div className="grid grid-cols-2 gap-3">
        {methodsToList.map(method => {
          ...
          } else if (colorClass.includes("purple") || colorClass.includes("indigo")) {
            bgClass = "bg-purple-900/20 border-purple-500/30";
          } else if (colorClass.includes("pink") || colorClass.includes("red")) {
